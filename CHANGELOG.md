...
### 新功能
- **遊戲內 YouTube 搜尋** — 浮窗右鍵選單或主視窗一鍵開啟，使用系統 Edge 引擎（WebView2），always-on-top 深色介面，支援中英文搜尋
- **排名背景圖右鍵開關** — 主視窗右鍵選單可隨時切換排名區背景圖顯示/隱藏，設定自動持久化

### 遊戲適配（05-27 更新）
- **新聖域「穆斯費爾聖杯」正確識別**
- **新聖域「穆斯費爾聖杯」BOSS 正確收錄 BOSS 模式** — 伊斯卡利爾、凱德利斯、守護劍
- **新超越副本 BOSS 正確收錄 BOSS 模式** — 羅塔爾、羅普斯提諾、克羅梅德深淵（Grade 1-4 共 12 個 NPC ID）

### 視覺調整
- **排名高亮重設計** — 量條統一採用遊戲官方青色（`#2b7375`），本人暱稱以亮綠色標示、其他人為白色，不再依賴微妙的色差區分
- **官方標準色導入** — DPS/傷害數值改用遊戲官方數值色（`#04e8e6`），統一套用至浮窗、排名面板、戰鬥細節面板
- **金屬字渲染修正** — 修復特定背景下暱稱文字出現方框殘影的問題，同時提升渲染效能
- **量條顏色調暗** — 職業色量條混合 30% 黑色，降低與文字的干擾
- **主視窗背景不透明度提升** — 94% 不透明，減少穿透干擾
- **伺服器名稱可讀性提升** — 改用亮灰色，與暱稱色彩層次更分明

### 內部改進
- **版本號統一來源** — 所有版本顯示集中管理，未來版本更新只需改一處
...
### 穩定性強化
- **視窗標題讀取穩定性修復** — 角色辨識功能（Window Title Lock）修正打包後未正常運作的問題，現已穩定運行
