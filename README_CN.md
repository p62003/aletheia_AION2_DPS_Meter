...
- 无边框窗口 + 边界吸附 — 拖曳靠近游戏窗口边缘会自动贴齐
- 标题列控制钮：重置／个人战报／展开主窗口（v9.0）
- **本人「YOU」标记** — 排名列中「你」的昵称胶囊右上角斜挂 YOU 标记，一眼辨识自己（v9.1）
- **BOSS 汇总行** — BOSS 战排名首行显示 BOSS 名称＋全队总伤害＋全队 DPS，红色满条区隔；标题列精简为计时＋击杀标记（v9.3）
- 半透明浮窗，背景透明度仅影响底图（文字保持不透明）
- Normal / Mini 两种尺寸切换（右键菜单）
- 右键菜单整合自定义功能（尺寸/伤害格式/DPS格式/透明度/背景图/主题）
- **Hover 技能面板** — 鼠标悬停排名行即时弹出完整技能明细
- **技能心电图 Cast ECG** — 波形图呈现技能施放节奏（波高=伤害量、波色=衔接速度）
- **状态分布图** — buff 施放时序 Gantt + 次数胶囊，依方向分色（v9.0）
- **伤害类型胶囊** — 七种伤害类型占比（暴击／强击／完美／前方／后方／格挡／多段），对齐官方水表（v9.0）
...
| 全队总览 + 框选统计 | 放大查看技能细节 |
|:---:|:---:|
| ![Skill MAP 总览](images/tool_skillmap_main.png) | ![Skill MAP 细节](images/tool_skillmap_main2.png) |

### 战斗分析
- 技能明细：伤害占比、暴击率、平均命中、特化灯号
- **玩家本人装备评分** — 战斗细节显示玩家本人的装备评分（v9.1）
- 技能时间轴：施放顺序记录，确认操作手法与连招
- **cHPS 治愈统计** — 战斗细节、战报、分析工具全面支持治愈量统计
- **个人战报窗口** — 浮窗一键开启最近 20 场本人战报与完整战斗细节（v9.0）
- 战报系统：BOSS／计时模式结算自动产生战报
- **战报管理** — 浏览、筛选（职业/cDPS/cHPS 范围 + 目标）、批量删除
- **战报面板** — 主窗口侧边抽屉，搜索/筛选/上传一站完成
- **战报上传** — 一键上传至永恒蜂窝或 Aletheia 社群，其他玩家自动匿名化（v9.0）
- **完整战报上传** — 上传至 Aletheia 社群的战报现在携带每个技能的完整指标与治愈量，供职业技能榜与特化／烙印统计（v9.1）
- **免费战报自动上传** — 免费用户的合规战报匿名自动上传至 Aletheia 社群（需启动时同意条款）；Sponsor 预设关闭、保留手动选择目标；队友永远匿名（v9.3）
...
| 战报分析（竞速模式） | 战报管理 |
|:---:|:---:|
| ![战报分析 竞速](images/analyze_tool_zh3.png) | ![战报管理](images/tool_Manager_zh.png) |

| 网卡选择 | 环境诊断 |
|:---:|:---:|
| ![网卡选择](images/interface_zh.png) | ![环境诊断](images/tool_net_report_zh.png) |
...
### 其他
- **内置自动更新** — 以 ALETHEIA-DPS.exe 启动；之后版本自动套用差异更新、免整包重载，更新中断可自动恢复；公告窗口另有「检查更新」按钮（v9.1）
- **新战报／新版本红点提示** — 主窗口、侧栏、右键菜单、系统匣同步显示（v9.3）
...
3. 解压缩后，以 **ALETHEIA-DPS.exe** 启动（右键 → 以管理员身份运行）— v9.1 起为启动入口，内置自动更新
4. 首次启动需以 Discord 帐号登入并同意战报上传条款才能进入主窗口。可选择输入 `AION2-XXXX-XXXX` 序号启用 Pro — 未启用时主窗口会被模糊遮罩，但浮窗与基础监控仍可使用。
5. 进入游戏即可看到数据
...
A: v8.0 包含连续技分组、DOT 分类、技能合并及解析精度提升，伤害数据更加精确。神石伤害、召唤物伤害均已计入总量。v9.0 并将伤害协议（强击／暴击／后方／多段）与官方水表完全对齐。

**Q: 独占全屏幕下看不到浮窗？**

A: 独占全屏幕会绕过 Windows DWM 合成层，所有置顶浮窗（含 Discord／Steam／NVIDIA）都会失效。请改用「无边框全屏幕／视窗化全屏幕」，视觉效果等同全屏幕且浮窗可正常显示。
