...
- **本人「YOU」標記** — 排名列中「你」的暱稱膠囊右上角斜掛 YOU 標記，一眼辨識自己（v9.1）
- **BOSS 彙總行** — BOSS 戰排名首行顯示 BOSS 名稱＋全隊總傷害＋全隊 DPS，紅色滿條區隔；標題列精簡為計時＋擊殺標記（v9.3）
...
- **完整戰報上傳** — 上傳至 Aletheia 社群的戰報現在攜帶每個技能的完整指標與治癒量，供職業技能榜與特化／烙印統計（v9.1）
- **免費戰報自動上傳** — 免費用戶的合規戰報匿名自動上傳至 Aletheia 社群（需啟動時同意條款）；Sponsor 預設關閉、保留手動選擇目標；隊友永遠匿名（v9.3）
...
### 其他
- **內建自動更新** — 以 ALETHEIA-DPS.exe 啟動；之後版本自動套用差異更新、免整包重載，更新中斷可自動回復；公告視窗另有「檢查更新」按鈕（v9.1）
- **新戰報／新版本紅點提示** — 主視窗、側欄、右鍵選單、系統匣同步顯示（v9.3）
...
3. 解壓縮後，以 **ALETHEIA-DPS.exe** 啟動（右鍵 → 以系統管理員身份執行）— v9.1 起為啟動入口，內建自動更新
4. 首次啟動需以 Discord 帳號登入並同意戰報上傳條款才能進入主視窗。可選擇輸入 `AION2-XXXX-XXXX` 序號啟用 Pro — 未啟用時主視窗會被模糊遮罩，但浮窗與基礎監控仍可使用。
5. 進入遊戲即可看到數據
